...
async function loadNews() {
    ...
    try {
                ...
                <div class="w-full flex items-center justify-between p-6 space-x-6">
                    <div class="flex-1 truncate">
                        <div class="flex items-center space-x-3">
                            ...
                            <span class="flex-shrink-0 inline-block px-2 py-0.5 text-${item.categoryColor}-800 text-xs leading-4 font-medium bg-${item.categoryColor}-100 rounded-full">${item.category}</span>
                        </div>
                        <p class="mt-1 text-gray-500 text-sm leading-5 truncate">${item.summary}</p>
                    </div>
                </div>
            </li>
        `).join('');
        ...
        newsContainer.innerHTML = '<p class="text-center text-red-500">Failed to load news. Ensure db.json is served correctly.</p>';
    }
}
